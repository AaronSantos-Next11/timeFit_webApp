...
const ModalNote = ({ open, onClose, modoEdicion, notaEditar, onGuardadoExitoso }) => {
  ...
  useEffect(() => {
    if (modoEdicion && notaEditar) {
      setForm({
        title: notaEditar.title || '',
        content: notaEditar.content || '',
        category: notaEditar.category || 'nota'
      });
    } else {
      setForm({
        title: '',
  ...
  const handleSave = () => {
    ...
    const noteData = {
      ...form,
      ...(modoEdicion && notaEditar && { id: notaEditar.id }),
      ...(modoEdicion && notaEditar && { createdAt: notaEditar.createdAt })
    };

    onGuardadoExitoso(noteData);
    onClose();
  };
    ...
    >
        ...
        <Box sx={{ 
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Typography variant="body2" sx={{ color: 'rgba(3, 3, 3, 0.8)', fontWeight: 'bold' }}>
              {modoEdicion ? 'Modificar los datos de la nota' : 'Crear una nueva nota para guarda tus ideas creativas o recordatorios importantes'}
            </Typography>
          </Box>
          <IconButton 
            onClick={onClose}
      ...
      <DialogActions sx={{ 
        ...
        <Button 
          onClick={handleSave} 
          variant="contained"
          startIcon={modoEdicion ? <EditIcon /> : <AddIcon />}
          sx={{ 
            background: '#F8820B',
            color: 'black',
            fontWeight: 'bold',
